...
<template>
        ...
        <v-col cols="12" sm="8" md="6">
          <v-card elevation="2">
            ...
            <v-card-text>
              ...
              <v-form ref="form" @submit.prevent="PasswordUpdate">
              <v-text-field
                v-model="passwordNew"
                label="새 비밀번호"
                type="password"
  ...
  const passwordRules = [
  v => !passwordNew || v.length >= 6 || '새 비밀번호는 최소 6자리 이상이어야 합니다',
  ]
  ...
  async function PasswordUpdate() {
    ...
    try {
      // Swagger 문서의 PATCH /user/ 사용
      const { pwdata } = await axios.post('http://127.0.0.1:8000/accounts/password/change/', {
        new_password1: passwordNew.value,
        new_password2: passwordConfirm.value,
      })
      // Pinia에 저장된 user 정보 동기화
      // const { userdata } = await axios.get('http://127.0.0.1:8000/accounts/user/')
      // auth.user = userdata
      router.push({ name: 'home' })
    } catch (e) {
      console.log(e)
